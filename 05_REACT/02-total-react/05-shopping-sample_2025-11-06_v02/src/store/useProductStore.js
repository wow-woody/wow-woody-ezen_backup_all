...
export const useProductStore = create((set, get) => ({
    ...
    onAddWishList: (product) => {
        ...
        // wishLists에 들어있는 제품 중, 방금 찜한 제품이 들어있는지 확인
        // map, filter, find
        //map : 모든 요소를 돌면서 새로운 배열 생성 - 데이터를 변형을 줘서 다시 배열 생성
        // filter : 조건에 맞는 요소만 찾아서 새로운 배열 생성
        // find : 조건에 맞는 요소가 있는지 확인 (true/false)

        const existing = wish.find((wish) =>
            wish.id === product.id
        );

        // 같은 제품이 있으면 찜목록에 추가하지 않고, "이미 있는 제품입니다"를 출력하기
        // 없으면 새롭게 추가하기
        let updateWish;
